...
function export_db_params(){
    ...
    export SHARED_DATABASE_URL=$(jq -r '.jdbc[] | select ( .name == '\"${db_name}\"' ) | .database[] | select ( .name == "WSO2AM_SHARED_DB") | .url' ${INFRA_JSON})
    export SHARED_DATABASE_USERNAME=$(jq -r '.jdbc[] | select ( .name == '\"${db_name}\"' ) | .database[] | select ( .name == "WSO2AM_SHARED_DB") | .username' ${INFRA_JSON})
    export SHARED_DATABASE_PASSWORD=$(jq -r '.jdbc[] | select ( .name == '\"${db_name}\"' ) | .database[] | select ( .name == "WSO2AM_SHARED_DB") | .password' ${INFRA_JSON})
    ...
    export API_MANAGER_DATABASE_URL=$(jq -r '.jdbc[] | select ( .name == '\"${db_name}\"' ) | .database[] | select ( .name == "WSO2AM_APIM_DB") | .url' ${INFRA_JSON})
    export API_MANAGER_DATABASE_USERNAME=$(jq -r '.jdbc[] | select ( .name == '\"${db_name}\"' ) | .database[] | select ( .name == "WSO2AM_APIM_DB") | .username' ${INFRA_JSON})
    export API_MANAGER_DATABASE_PASSWORD=$(jq -r '.jdbc[] | select ( .name == '\"${db_name}\"' ) | .database[] | select ( .name == "WSO2AM_APIM_DB") | .password' ${INFRA_JSON})
...
wget https://raw.githubusercontent.com/PasanT9/apim-test-integration/4.3.0-copy/infra.json
...
if [[ $DB_TYPE = "mysql" ]]; then
    ...
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -e "DROP DATABASE IF EXISTS WSO2AM_SHARED_DB"
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -e "DROP DATABASE IF EXISTS WSO2AM_APIM_DB"
    ...
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -e "CREATE DATABASE WSO2AM_SHARED_DB CHARACTER SET latin1"
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -e "CREATE DATABASE WSO2AM_APIM_DB CHARACTER SET latin1"
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -e "CREATE DATABASE WSO2AM_STAT_DB CHARACTER SET latin1"

    log_info "[Mysql] Povisioning WSO2AM_APIM_DB"
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -D WSO2AM_APIM_DB <  $DB_SCRIPT_PATH/apimgt/mysql.sql
    log_info "End [Mysql] Povisioning WSO2AM_APIM_DB"

    log_info "[Mysql] Povisioning WSO2AM_SHARED_DB"
    mysql -u $CF_DB_USERNAME -p$CF_DB_PASSWORD -h $CF_DB_HOST -P $CF_DB_PORT -D WSO2AM_SHARED_DB <  $DB_SCRIPT_PATH/mysql.sql
    log_info "End [Mysql] Povisioning WSO2AM_SHARED_DB"
    ...
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d postgres -c "DROP DATABASE IF EXISTS \"WSO2AM_SHARED_DB\""
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d postgres -c "DROP DATABASE IF EXISTS \"WSO2AM_APIM_DB\""
    ...
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d postgres -c "CREATE DATABASE \"WSO2AM_SHARED_DB\""
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d postgres -c "CREATE DATABASE \"WSO2AM_APIM_DB\""
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d postgres -c "CREATE DATABASE \"WSO2AM_STAT_DB\""

    log_info "[Postgres] Provisioning database WSO2AM_APIM_DB"
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d WSO2AM_APIM_DB -f $DB_SCRIPT_PATH/apimgt/postgresql.sql
    log_info "[Postgres] Provisioning database WSO2AM_SHARED_DB"
    psql -U $CF_DB_USERNAME -h $CF_DB_HOST -p $CF_DB_PORT -d WSO2AM_SHARED_DB -f $DB_SCRIPT_PATH/postgresql.sql
    ...
    echo "DECLARE USER_EXIST INTEGER;"$'\n'"BEGIN SELECT COUNT(*) INTO USER_EXIST FROM dba_users WHERE username='WSO2AM_APIM_DB';"$'\n'"IF (USER_EXIST > 0) THEN EXECUTE IMMEDIATE 'DROP USER WSO2AM_APIM_DB CASCADE';"$'\n'"END IF;"$'\n'"END;"$'\n'"/" > apim_oracle_user.sql
    ...
    echo "CREATE USER WSO2AM_SHARED_DB IDENTIFIED BY $CF_DB_PASSWORD;"$'\n'"GRANT CONNECT, RESOURCE, DBA TO WSO2AM_SHARED_DB;"$'\n'"GRANT UNLIMITED TABLESPACE TO WSO2AM_SHARED_DB;" >> apim_oracle_user.sql
    echo "CREATE USER WSO2AM_APIM_DB IDENTIFIED BY $CF_DB_PASSWORD;"$'\n'"GRANT CONNECT, RESOURCE, DBA TO WSO2AM_APIM_DB;"$'\n'"GRANT UNLIMITED TABLESPACE TO WSO2AM_APIM_DB;" >> apim_oracle_user.sql
    ...
    echo exit | sqlplus64 'WSO2AM_SHARED_DB/$CF_DB_PASSWORD@(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(Host=$CF_DB_HOST)(Port=$CF_DB_PORT))(CONNECT_DATA=(SID=WSO2AMDB)))' @$DB_SCRIPT_PATH/oracle.sql
    echo exit | sqlplus64 'WSO2AM_APIM_DB/$CF_DB_PASSWORD@(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(Host=$CF_DB_HOST)(Port=$CF_DB_PORT))(CONNECT_DATA=(SID=WSO2AMDB)))' @$DB_SCRIPT_PATH/apimgt/oracle.sql
    ...
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "DROP DATABASE IF EXISTS WSO2AM_SHARED_DB"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "DROP DATABASE IF EXISTS WSO2AM_APIM_DB"

    log_info "[SQLServer] Creating Databases"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "CREATE DATABASE WSO2AM_SHARED_DB"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "CREATE DATABASE WSO2AM_APIM_DB"

    log_info "[SQLServer] Provisioning database WSO2AM_APIM_DB"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -d WSO2AM_APIM_DB -i $DB_SCRIPT_PATH/apimgt/mssql.sql
    log_info "[SQLServer] Provisioning database WSO2AM_SHARED_DB"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -d WSO2AM_SHARED_DB -i $DB_SCRIPT_PATH/mssql.sql
    log_info "[SQLServer] Tuning databases"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "ALTER DATABASE WSO2AM_APIM_DB  SET ALLOW_SNAPSHOT_ISOLATION ON"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "ALTER DATABASE WSO2AM_APIM_DB SET READ_COMMITTED_SNAPSHOT ON WITH ROLLBACK IMMEDIATE"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "ALTER DATABASE WSO2AM_SHARED_DB  SET ALLOW_SNAPSHOT_ISOLATION ON"
    sqlcmd -S $CF_DB_HOST -U $CF_DB_USERNAME -P $CF_DB_PASSWORD -Q "ALTER DATABASE WSO2AM_SHARED_DB SET READ_COMMITTED_SNAPSHOT ON WITH ROLLBACK IMMEDIATE"
